...
class BillingSchemaMigrationTests {
  ...
  private static final String SCHEMA = "billing_service";
  ...
  void setUp() throws SQLException {
    ...
    try {
      ...
      Assumptions.assumeTrue(
          false,
          "Skipping billing schema migration validation because Docker/Testcontainers is unavailable: "
  ...
  void createsExpectedTables() throws SQLException {
    assertThat(queryForLong(
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'billing_service'"))
        .isEqualTo(6);
  }

  @Test
  void seedsPlansSubscriptionEntitlementsAndUsage() throws SQLException {
    assertThat(queryForLong("SELECT COUNT(*) FROM plans"))
        .isEqualTo(4);
    assertThat(queryForString(
        "SELECT p.code FROM subscriptions s JOIN plans p ON p.id = s.plan_id "
            + "WHERE s.workspace_id = '10000000-0000-0000-0000-000000000001'"))
        .isEqualTo("pro");
    assertThat(queryForLong("SELECT COUNT(*) FROM usage_counters"))
        .isEqualTo(2);
  }
