...
class SchedulerSchemaMigrationTests {

  private static final String SCHEMA = "scheduler_service";
  ...
  void setUp() throws SQLException {
    ...
    try {
      ...
      Assumptions.assumeTrue(
          false,
          "Skipping scheduler schema migration validation because Docker/Testcontainers is unavailable: "
  ...
  void createsExpectedTables() throws SQLException {
    assertThat(queryForLong(
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'scheduler_service'"))
        .isEqualTo(11);
  }

  @Test
  void seedsDraftVariantsAssetsAndSchedulingState() throws SQLException {
    assertThat(queryForLong(
        "SELECT COUNT(*) FROM post_variants WHERE draft_id = "
    ...
    assertThat(queryForString(
        "SELECT status FROM scheduled_jobs WHERE draft_id = "
            + "'40000000-0000-0000-0000-000000000021'"))
        .isEqualTo("queued");
    assertThat(queryForLong(
        "SELECT COUNT(*) FROM variant_targets WHERE variant_id = "
