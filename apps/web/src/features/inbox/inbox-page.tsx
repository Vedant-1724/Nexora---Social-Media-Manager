...
export function InboxPage() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h2 className="font-display text-2xl text-slate-950">Smart Inbox</h2>
            {unreadCount > 0 && (
          ...
          <p className="mt-1 text-sm text-slate-500">Manage conversations, reviews, and customer interactions across all channels.</p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 border-b border-slate-200/60 overflow-x-auto pb-0">
        {TABS.map(tab => {
          ...
          return (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={cn("inbox-tab flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold whitespace-nowrap transition-colors",
                activeTab === tab.key ? "text-sky-700" : "text-slate-400 hover:text-slate-600"
              )} data-active={activeTab === tab.key}>
              {tab.icon}{tab.label}
              <span className={cn("rounded-md px-1.5 py-0.5 text-[10px] font-bold", activeTab === tab.key ? "bg-sky-100 text-sky-700" : "bg-slate-100 text-slate-500")}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr_380px]" style={{ minHeight: "calc(100vh - 340px)" }}>
        ...
        <div className="space-y-3">
          {/* Search */}
          <div className="flex items-center gap-2 rounded-2xl border border-slate-200/60 bg-white/60 px-3 py-2.5 backdrop-blur transition-all hover:border-sky-200 focus-within:border-sky-300 focus-within:ring-2 focus-within:ring-sky-500/20">
            <Search className="h-4 w-4 text-slate-400" />
            <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent text-sm outline-none placeholder:text-slate-400" placeholder="Search messages..." />
          </div>

          {/* Quick Stats */}
          <Card className="!p-4 !rounded-2xl">
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center"><p className="font-display text-xl font-semibold text-slate-950">{unreadCount}</p><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Unread</p></div>
              <div className="text-center"><p className="font-display text-xl font-semibold text-slate-950">{mockMessages.filter(m => m.assignedToUserId).length}</p><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Assigned</p></div>
              <div className="text-center"><p className="font-display text-xl font-semibold text-emerald-600">{mockMessages.filter(m => m.sentiment === "positive").length}</p><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Positive</p></div>
              <div className="text-center"><p className="font-display text-xl font-semibold text-red-500">{mockMessages.filter(m => m.sentiment === "negative").length}</p><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Negative</p></div>
            </div>
          </Card>
          ...
          <Card className="!p-4 !rounded-2xl">
            ...
            <div className="space-y-2.5">
              <select value={networkFilter} onChange={e => setNetworkFilter(e.target.value)}
                className="w-full rounded-xl border border-slate-200/60 bg-white/60 px-3 py-2 text-xs focus:border-sky-300 focus:outline-none">
            ...
            {(statusFilter !== "all" || sentimentFilter !== "all" || networkFilter !== "all" || tagFilter !== "all") && (
              <button onClick={() => { setStatusFilter("all"); setSentimentFilter("all"); setNetworkFilter("all"); setTagFilter("all"); }}
                className="mt-3 text-[10px] font-semibold text-sky-600 hover:text-sky-700">Clear All Filters</button>
            )}
          </Card>
          ...
          <Card className="!p-3 !rounded-2xl">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <button className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-semibold text-slate-600 hover:bg-sky-50 hover:text-sky-700 transition-colors">
                <CheckCheck className="h-3 w-3" />Mark Read
              </button>
              <button className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-semibold text-slate-600 hover:bg-slate-100 transition-colors">
                <Archive className="h-3 w-3" />Archive
              </button>
              <button className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-semibold text-slate-600 hover:bg-violet-50 hover:text-violet-700 transition-colors">
                <Tag className="h-3 w-3" />Tag
              </button>
              <button className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-semibold text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
        ...
        <Card className="!p-0 !rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <p className="text-xs font-semibold text-slate-600">{filtered.length} conversation{filtered.length !== 1 ? "s" : ""}</p>
            <Badge className="text-[10px]"><Sparkles className="h-2.5 w-2.5 mr-1" />Live</Badge>
          </div>
                ...
                {filtered.map(msg => {
                  ...
                  return (
                    <div key={msg.messageId} onClick={() => setSelectedId(msg.messageId)}
                      className={cn("inbox-message-row flex items-start gap-3 px-4 py-3.5 border-b border-slate-50", isActive && "!bg-sky-50/60")}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <p className={cn("text-sm truncate", msg.status === "unread" ? "font-bold text-slate-950" : "font-semibold text-slate-700")}>
                        ...
                        <p className={cn("text-xs mt-0.5 line-clamp-2", msg.status === "unread" ? "text-slate-600" : "text-slate-400")}>
                          {msg.body}
                        </p>
